...
docker-generate-cmd:
	...
	docker run -it --rm -u `id -u`:`id -g` -v `pwd`:/app -w /app --entrypoint java swaggerapi/swagger-codegen-cli -jar /opt/swagger-codegen-cli/swagger-codegen-cli.jar generate -t ./templates -i api.yaml -l go -c gen-config.json
	rm api.yaml
