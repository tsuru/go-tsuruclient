 ...
 * @param "File" (optional.Interface of *os.File) -  App's source files (tarball compressed with gzip).  NOTE: Cannot be presented mutually with `archive-url` or `image`.
 * @param "ArchiveUrl" (optional.String) -  HTTP URL to app's source files.  NOTE 1: Tsuru API must be able to download the file over HTTP and the downloaded file must be a tarball compressed with gzip.  NOTE 2: Cannot be presented mutually with `file` or `image`.  Example: `https://my-org.example.com/my-app/v42.tar.gz`.
